...
- `oldaplib/ontologies/shared.trig` version 0.4.0 implements the archive model in both SHACL and OWL: one generic `shared:ArchiveUnit`, seven fixed `shared:ArchiveLevel` named individuals, a single-parent adjacency tree, explicit media links, and optional description metadata (`dcterms:temporal` as `oldap:Dating`, multilingual `schema:materialExtent`, multi-valued `dcterms:creator` to `dcterms:Agent`, multilingual `dcterms:provenance`, and multilingual `schema:conditionsOfAccess`). Archive phases 1–3 are complete: generic CRUD/search remains the default, while `oldaplib/src/archive_tree.py` adds only ancestor-path traversal and cycle-safe moves with optional sibling positions. Access-condition text is descriptive only and never replaces OLDAP permissions. Structural and GraphDB integration tests cover ontology alignment, metadata round-tripping, the reference tree, non-empty deletion protection, paths, moves, and cycle rejection. `ArchivStruktur.md` records the decisions and incremental roadmap; the `shared:Item`/Fasnacht object boundary, strict hierarchy profiles, permission inheritance, and standards mappings remain deferred.
...
- Resolve the `shared:Item`/`fasnacht:ArchiveObject`/`fasnacht:ArchiveMediaObject` boundary before integrating existing Fasnacht archive records into the generic tree; keep hierarchy profiles, physical location, and permission inheritance deferred.
