...
class ExternalOntology(Model, Notify):
    ...
    def create_shacl(self, *,
        ...
        """Build a complete SPARQL update that creates this SHACL record.

        Args:
            timestamp: Creation and modification timestamp for the record.
            indent: Initial indentation level of the generated update.
            indent_inc: Number of spaces per indentation level.

        Returns:
            A complete ``INSERT DATA`` update containing the project SHACL
            graph and this external-ontology record.
        """
        blank = ''
        sparql = ''
        sparql += f'{blank:{indent * indent_inc}}INSERT DATA {{\n'
        sparql += f'{blank:{(indent + 1) * indent_inc}}GRAPH {self.__projectShortName}:shacl {{\n'
        sparql += self.create_shacl_triples(
            timestamp=timestamp,
            indent=indent + 2,
            indent_inc=indent_inc,
        )
        sparql += f'{blank:{(indent + 1) * indent_inc}}}}\n'
        sparql += f'{blank:{indent * indent_inc}}}}\n'
        return sparql

    def create_shacl_triples(self, *,
                             timestamp: Xsd_dateTime,
                             indent: int = 0,
                             indent_inc: int = 4) -> str:
        """Build only the SHACL triples for this external ontology.

        This fragment deliberately contains neither ``INSERT DATA`` nor a
        ``GRAPH`` clause. It can therefore be embedded safely in the combined
        update generated by :class:`DataModel`.

        Args:
            timestamp: Creation and modification timestamp for the record.
            indent: Initial indentation level of the generated triples.
            indent_inc: Number of spaces per indentation level.

        Returns:
            RDF statements describing the external ontology.
        """
        blank = ''
        sparql = f'{blank:{indent * indent_inc}}{self.__extonto_qname.toRdf} a oldap:ExternalOntology'
